...
def main(title, braid, degree, weight, inversion_data=None, alexander_test=False):
    print(f"Processing knot {title} with braid {braid} and degree {degree} and weight {weight}")
    bs = BraidStates(braid)
    if inversion_data is None:
        inversion_data = [1] * bs.n_s_total
...
if __name__ == "__main__":
     ...
     braids = [
         [k for k in [1,3,2,-3, 2,1,-2,3,3,2,-3,2,3]],
     ]
     titles = [
             f"12n_801_xdeg_{DEGREE}_wt_{WEIGHT}_sqn_braid"
     ]
     inversion_data = [
         None
     ]
     for item in list(zip(titles, braids, inversion_data)):
         main(item[0], item[1], DEGREE, WEIGHT, item[2]) # tip: C++ output file will not be produced if ILP is unbounded, which can happen if weight is -1 and knot needs stratified summation
