...
class ForecastTableGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Profiling Forecast Table Viewer")
    ...
    def create_control_panel(self, parent):
        ...
        ttk.Button(file_frame, text="Load Profiling File",
    ...
    def load_default_data(self):
        """Try to load default profiling_results.txt file"""
        try:
            self.data = parse_profiling_data('profiling_results.txt')
            if self.data:
                self.file_label.config(text="profiling_results.txt")
    ...
    def load_file(self):
        ...
        filename = filedialog.askopenfilename(
            title="Select Profiling Results File",
            filetypes=[("Text files", "*.txt"), ("All files", "*.*")]
        )

        if filename:
            try:
                self.data = parse_profiling_data(filename)
                if self.data:
    ...
    def update_plot(self):
        ...
        try:
            ...
            self.ax.set_xlabel('Degree')
            self.ax.set_ylabel('Time (seconds)')
            self.ax.set_title(f'Runtime Analysis - Braid Length {self.current_braid_length}')
            self.ax.set_yscale('log')
            self.ax.grid(True, alpha=0.3)
            self.ax.legend()
