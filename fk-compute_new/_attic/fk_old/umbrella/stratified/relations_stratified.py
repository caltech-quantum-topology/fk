...
from stateliteral import StateLiteral, ZERO_STATE, NUNITY_STATE
...
def check_sign_assignment(degree, weight, relations, braid_states):
...
def czech_sign_assignment(degree, weight, relations, braid_states):
    assignment = symbolic_variable_assignment(relations, braid_states)
    criteria, multiples, singlesigns = process_assignment(assignment, braid_states, relations)
    for (key, value) in criteria.items():
        if key >= 0:
...
def ilp(degree, weight, relations, braid_states, write_to):

    check = czech_sign_assignment(degree, weight, relations, braid_states)
    if check is None:
        return None
...
from relations_stratified import full_reduce, ilp
...
def FK(braid, degree, weight, outfile, inversion_data=None):
    ...
    if braid_type_ == BRAID_TYPE_HOMEGENOUS:
        ...
        solution = ilp(degree, weight, relations, braid_states, outfile)
        if solution == None:
