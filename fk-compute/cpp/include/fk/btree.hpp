    ...
    private:
        ...
        void updateNodeRecursively (std::vector<T> inputVector, int vectorSize) {
            if (vectorSize != 0) {
                ...
                btree<T>* childTreePointer = new btree<T>; // need to use custom resource management so children don't get deleted when leaving "update" member function's scope; ADD CUSTOM DESTRUCTOR, AS WELL! CURRENTLY, ONLY ROOT GETS DELETED WHEN SCOPE ITS INITIATED IN ENDS
        ...
        bool searchNodeRecursively (std::vector<T> inputVector, int vectorSize) {
            if (vectorSize != 0) {
                ...
                for (int i = 0; i < childrenValues.size(); i++) {
                    if (childrenValues[i] == inputVector[vectorSize]) {
                        return (*childrenPointers[i]).searchNodeRecursively(inputVector, vectorSize);
                    }
                }
                return false;
            }
