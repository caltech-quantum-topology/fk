...
struct bilvector {
    ...
    public:
        bilvector (int initialNegativeVectorCount, int initialPositiveVectorCount, int componentSizeParam, T defaultValueParam) {
        ...
        int getNegativeSize () {
            return negativeVectorCount * componentSize;
        }
        int getNegativeSize () const {
            return negativeVectorCount * componentSize;
        }
        int getPositiveSize () {
            return positiveVectorCount * componentSize;
        }
        ...
        int getNegativeVectorCount() {
            return negativeVectorCount;
        }
        int getPositiveVectorCount() {
            return positiveVectorCount;
        }
        int getMaxNegativeIndex() {
            return maxNegativeIndex;
        }
        int getMaxNegativeIndex() const {
            return maxNegativeIndex;
        }
        int getMaxPositiveIndex() {
        ...
        const T& operator[] (int accessIndex) const {
            // For const access, we can't modify the structure, so we need to handle this carefully
            if (accessIndex >= 0) {
                if (accessIndex >= this->getPositiveSize()) {
