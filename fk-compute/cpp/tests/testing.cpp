...
void writeBilvectorToJson(std::vector<bilvector<int>>& bilvectorTerms, const std::string& outputFileName, const std::string& testName) {
    ...
    for (size_t i = 0; i < bilvectorTerms.size(); i++) {
        ...
        for (int j = bilvectorTerms[i].getMaxNegativeIndex(); j <= bilvectorTerms[i].getMaxPositiveIndex(); j++) {
...
int main() {
    ...
    // Test case 1: Simple case
    {
        ...
        std::cout << "Running computeXQPochhammer(term, 3, 1, 0, components, lengths, blocks)...\n";
        computeXQPochhammer(term, 3, 1, 0, components, lengths, blocks);

        writeBilvectorToJson(term, "test1_output.json", "computeXQPochhammer(3,1,0)");
    }
    ...
    // Test case 2: Different parameters
    {
        ...
        std::cout << "Running computeXQPochhammer(term, 2, 0, 1, components, lengths, blocks)...\n";
        computeXQPochhammer(term, 12, 1, 0, components, lengths, blocks);

        writeBilvectorToJson(term, "test2_output.json", "computeXQPochhammer(2,0,1)");
    }
    ...
    // Test case 3: Edge case with up = low
    {
        ...
        std::cout << "Running computeXQPochhammer(term, 1, 1, 0, components, lengths, blocks)...\n";
        computeXQPochhammer(term, 3, 1, 0, components, lengths, blocks);

        writeBilvectorToJson(term, "test3_output.json", "computeXQPochhammer(1,1,0)");
    }
