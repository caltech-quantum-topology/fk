...
int parseStringToInteger (std::string inputString) {
    ...
    for (int i = startIndex; i < inputString.size(); i++) {
        accumulator += (inputString[i] - '0') * std::pow(10, inputString.size() - i - 1);
    }
    if (isNegative) {
...
double parseStringToDouble (std::string inputString) {
    ...
    for (int i = startIndex; i < decimalPointIndex; i++) {
        accumulator += (inputString[i] - '0') * std::pow(10, decimalPointIndex - i - 1);
    }
    for (int i = decimalPointIndex + 1; i < inputString.size(); i++) {
        accumulator += (inputString[i] - '0') * std::pow(10, -(i - decimalPointIndex));
    }
    if (isNegative) {
        return -1 * accumulator;
    }
