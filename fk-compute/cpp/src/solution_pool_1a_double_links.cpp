...
void recurse_2(std::vector<std::vector<double>> &criteria,
  ...
  if (bounds.size()) {
    ...
    for (int i = 0; i <= upper; i++) {
      point[index] = i;
      recurse_2(criteria, bounds, supporting_inequalities, point, function);
    }
  } else {
    bool cond = true;
...
void recurse_1(std::vector<std::vector<double>> &new_criteria,
  ...
  if (first.size()) {
    ...
    for (int i = 0; i <= degrees[main_index] / slope; i++) {
      ...
      recurse_1(new_criteria, new_degrees, criteria, first, bounds,
...
void pooling(std::vector<std::vector<double>> main_inequalities,
  ...
  if (bounded > 0) {
    ...
    if (bounded == size - 1) {
      ...
      recurse_1(main_inequalities, degrees, main_inequalities, first, bounds,
                supporting_inequalities, point, function);
      return;
    }
    int index = 0;
          ...
          if (supporting_inequalities[l][1 + index] < 0) {
            ...
            if (useful) {
              ...
              if (bounded == size - 1) {
                ...
                std::vector<int> point(size - 1, 0);
                recurse_1(main_inequalities, degrees, main_inequalities, first,
                          bounds, supporting_inequalities, point, function);
                return;
              }
              index = -1;
              break;
  ...
  std::vector<std::vector<double>> new_criteria(mains,
        ...
        for (int j = 1; j < size; j++) {
          if (criteria[q][j] > 0 && supporting_inequalities[i][j] < 0) {
            for (int k = 0; k < size; k++) {
              new_criteria[q][k] =
                  criteria[q][k] + supporting_inequalities[i][k] / 2.0;
            }
              ...
              if (!visited[visdex].containsVector(new_criteria[visdex])) {
                ...
                if (bounded > 0) {
                  ...
                  if (bounded == size - 1) {
                    ...
                    std::vector<int> point(size - 1, 0);
                    recurse_1(new_criteria, degrees, criteria, first, bounds,
                              supporting_inequalities, point, function);
                    return;
                  }
                  int index = 0;
                    ...
                    if (!bounded_v[index]) {
                      for (int l = 0; l < support; l++) {
                        if (supporting_inequalities[l][1 + index] < 0) {
                          ...
                          for (int n = 0; n < size - 1; n++) {
                            if (n != index &&
                                supporting_inequalities[l][1 + n] > 0 &&
                                !bounded_v[n]) {
                              useful = false;
                            }
                          }
                          if (useful) {
                            ...
                            if (bounded == size - 1) {
                              ...
                              std::vector<int> point(size - 1, 0);
                              recurse_1(new_criteria, degrees, criteria, first,
                                        bounds, supporting_inequalities, point,
                                        function);
                              return;
                            }
                            index = -1;
