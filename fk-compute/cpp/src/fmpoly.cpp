...
std::vector<bilvector<int>> FMPoly::getCoefficients() const {
  // Create vector with correct size for compatibility
  std::vector<bilvector<int>> result;

  // Calculate the total number of x-monomials needed
  int totalMonomials = 1;
  for (int degree : maxXDegrees) {
    totalMonomials *= (degree + 1);
  }

  result.resize(totalMonomials);

  // Initialize each bilvector with reasonable defaults
  for (int i = 0; i < totalMonomials; ++i) {
    result[i] = bilvector<int>(10, 10, 1, 0); // neg_size, pos_size, component_size, default_value
  }

  // If polynomial is zero, return empty coefficient vectors
  if (fmpz_mpoly_is_zero(poly, ctx)) {
    return result;
  }

  // Iterate through all terms in the FLINT polynomial
  slong len = fmpz_mpoly_length(poly, ctx);
  ...
  for (slong i = 0; i < len; ++i) {
    ...
    if (!fmpz_is_zero(coeff)) {
      ...
      fmpz *exps = _fmpz_vec_init(numXVariables + 1);
      fmpz_mpoly_get_term_exp_fmpz(exps, poly, i, ctx);

      // Extract q power and x powers
      int qPower, xIndex;
      std::vector<int> xPowers;
      if (getExponentsFromMonomial(exps, qPower, xPowers)) {
        // Calculate linear index for x-monomial
        xIndex = 0;
        for (int j = 0; j < numXVariables; ++j) {
          if (j < static_cast<int>(xPowers.size())) {
            xIndex += xPowers[j] * blockSizes[j];
          }
        }

        // Store coefficient in the appropriate bilvector
        if (xIndex >= 0 && xIndex < totalMonomials) {
          long coeffValue = fmpz_get_si(coeff);
          result[xIndex][qPower] = static_cast<int>(coeffValue);
        }
      }

      _fmpz_vec_clear(exps, numXVariables + 1);
    }
