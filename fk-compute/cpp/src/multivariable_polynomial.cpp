...
MultivariablePolynomial::MultivariablePolynomial(int numVariables, int degree,
    ...
    if (maxDegrees.empty()) {
        ...
        if (maxDegrees.size() != numVariables) {
            throw std::invalid_argument("Max degrees vector size must match number of variables");
        }
...
int MultivariablePolynomial::multiIndexToLinear(const std::vector<int>& xPowers) const {
    ...
    for (int i = 0; i < numXVariables; i++) {
        if (xPowers[i] < 0 || xPowers[i] > maxXDegrees[i]) {
            throw std::out_of_range("X power out of bounds for variable " + std::to_string(i));
        }
...
std::vector<int> MultivariablePolynomial::linearToMultiIndex(int linearIndex) const {
...
void MultivariablePolynomial::setCoefficient(int qPower, const std::vector<int>& xPowers, int coefficient) {
    int linearIndex = multiIndexToLinear(xPowers);
    coefficients[linearIndex][qPower] = coefficient;
}

void MultivariablePolynomial::addToCoefficient(int qPower, const std::vector<int>& xPowers, int coefficient) {
    int linearIndex = multiIndexToLinear(xPowers);
    coefficients[linearIndex][qPower] += coefficient;
}
...
const std::vector<bilvector<int>>& MultivariablePolynomial::getCoefficients() const {
    return coefficients;
}
...
void MultivariablePolynomial::exportToJson(const std::string& fileName) const {
    ...
    for (int i = 0; i < numXVariables; i++) {
        ...
        if (i < numXVariables - 1) outputFile << ",";
    }
    outputFile << "]\n";
    outputFile << "\t}\n}";
    outputFile.close();
...
void MultivariablePolynomial::print(int maxTerms) const {
    ...
    int termCount = 0;
    for (size_t linearIndex = 0; linearIndex < coefficients.size() && termCount < maxTerms; linearIndex++) {
        std::vector<int> xPowers = linearToMultiIndex(linearIndex);

        for (int j = coefficients[linearIndex].getMaxNegativeIndex();
             j <= coefficients[linearIndex].getMaxPositiveIndex() && termCount < maxTerms; j++) {
...
void MultivariablePolynomial::checkCompatibility(const MultivariablePolynomial& other) const {
    if (numXVariables != other.numXVariables) {
        throw std::invalid_argument("Polynomials must have the same number of x variables");
    }

    for (int i = 0; i < numXVariables; i++) {
        if (maxXDegrees[i] != other.maxXDegrees[i]) {
            throw std::invalid_argument("Polynomials must have the same maximum degrees for each variable");
        }
    }
}
...
MultivariablePolynomial& MultivariablePolynomial::operator+=(const MultivariablePolynomial& other) {
    ...
    for (size_t linearIndex = 0; linearIndex < coefficients.size(); linearIndex++) {
        ...
        for (int j = otherBilvec.getMaxNegativeIndex(); j <= otherBilvec.getMaxPositiveIndex(); j++) {
...
MultivariablePolynomial& MultivariablePolynomial::operator*=(const MultivariablePolynomial& other) {
    ...
    for (size_t thisLinearIndex = 0; thisLinearIndex < coefficients.size(); thisLinearIndex++) {
        std::vector<int> thisXPowers = linearToMultiIndex(thisLinearIndex);
        const auto& thisBilvec = coefficients[thisLinearIndex];

        for (size_t otherLinearIndex = 0; otherLinearIndex < other.coefficients.size(); otherLinearIndex++) {
            std::vector<int> otherXPowers = other.linearToMultiIndex(otherLinearIndex);
            ...
            if (withinBounds) {
                ...
                for (int thisQ = thisBilvec.getMaxNegativeIndex(); thisQ <= thisBilvec.getMaxPositiveIndex(); thisQ++) {
                    int thisCoeff = thisBilvec[thisQ];
                    if (thisCoeff != 0) {
                        for (int otherQ = otherBilvec.getMaxNegativeIndex(); otherQ <= otherBilvec.getMaxPositiveIndex(); otherQ++) {
...
MultivariablePolynomial operator+(const MultivariablePolynomial& lhs, const MultivariablePolynomial& rhs) {
